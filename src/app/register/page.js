...
export default function RegisterPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.message || "No fue posible registrar usuario");
      }
  ...
  return (
    <main className="min-h-screen bg-zinc-950 px-6 py-10 text-zinc-200">
      <div className="mx-auto max-w-md rounded-2xl border border-white/10 bg-white/5 p-8 shadow-[0_18px_40px_rgba(0,0,0,0.35)] backdrop-blur-md">
        <p className="text-xs uppercase tracking-[0.32em] text-amber-400">Registro</p>
        <h1 className="mt-3 text-3xl font-semibold text-zinc-100">Crear Cuenta</h1>
        ...
        <p className="mt-5 text-sm text-zinc-400">
          ¿Ya tienes cuenta?{" "}
          <Link className="text-amber-400 transition-colors hover:text-amber-300" href="/login">
            Inicia sesión aquí
          </Link>
        </p>
      </div>
